...
The conventional way to address this problem is with greater predictor size or complexity, but these are costs which cannot be afforded on area-constrained cores. This paper proposes that targeting the \textbf{predictor working set} is as effective as growing the predictor, and can deliver performance gains which match that of very large predictors while still using very small predictors. This paper introduces profile-guided memory dependence prediction (PG-MDP), a software co-design to label consistently memory independent loads via their opcode and remove them from the MDP working set. These loads are referred to as \emph{predict-no-dependency} (PND) loads, and bypass querying the MDP when dispatched, always issuing as soon as possible. Across SPEC CPU 2017 intspeed, PG-MDP reduces dynamic MDP queries by 77\%, false dependencies by 80\%, and improves IPC for a small simulated core by 1.47\% (to within 0.5\% of a very large predictor), all at \textbf{zero hardware cost}.
...
regen graphs
fill in sweep and power sections
...
\begin{itemize}
        ...
        \item[$\bullet$] \textbf{Closes Store Sets Table Size Gap For Free:} By shrinking the MDP working set to only memory dependent loads, PG-MDP allows a 64-entry XiangShan Store Set predictor to match the performance of 1024 entries within half a percent, achieving a geomean 1.47\% IPC gain on a small simulated core.
\end{itemize}
...
This section presents the experimental results of applying PG-MDP to SPEC2017 CPU intspeed workloads. We first highlight the effective elimination of false dependencies and the resulting IPC gain on a small core (ROB=128). We then demonstrate how the benefits of PG-MDP scale to larger cores and to state-of-the-art MDP algorithms such as PHAST. Next, we analyze how performance changes across varying XS \aj{reduced?, extra-small? Unify across all uses} Store Set table sizes. Finally, we estimate potential performance gains by simulating a limited number of MDP read ports to restrict the dispatch of non-labelled loads per cycle.

\subsection{Experimental Design}

-medium is still too small for phast
...
\subsection{Predictor Size Performance Gap}
...
Figure \ref{fig:fullssitsweep} shows an extended plot of the results in Figure \ref{fig:ssitsweep}, including the medium core configuration as well as the small core. The x-axis represents both the number of SSIT and LFST entries (after being multiplied by an appropriate number of slots). Both cases confirm our initial hypothesis that for a Store Set-like predictor, a small number of entries can capture most of the performance of a large number of entries for an appropriate predictor working set. Figure \ref{fig:lookups} shows the reduction in dynamic MDP queries, with an arithmetic average reduction of 77\%, demonstrating that only roughly 25\% of the original predictor working set needs to be tracked with hardware structures.

In both core size configurations, 64 table entries with PG-MDP achieves an IPC within 0.5\% of 1024 entries. In an area-constrained use case as in the small core, this buys the benefits of a larger predictor without any area cost. The medium core represents a case where more area can be spared, and so contains a predictor that already delivers most of the potential benefit. In this case, PG-MDP can be used to reduce the area cost while maintaining performance.
