...
\aj{Insight on the correctness of this technique.}
\aj{Correctness: PGO only bypasses prediction. Speculation is still checked, including when stemming from PGO.}
...
We use Gem5 to simulate various core size configurations based on parameters from real-world processors. Each core either utilises the Store Sets or PHAST predictor, depending on which provides optimal performance within the area budget for that configuration. Results indicate that Store Sets performs optimally in smaller area allocations, whereas PHAST is more accurate with larger areas. Additionally, we assess a highly constrained core using the S5 configuration which does not use a memory dependence predictor, and instead conservatively stalls loads on all unresolved stores. This analysis examines the potential impact on very small cores that still perform general-purpose computation, such as those in smartwatches.
